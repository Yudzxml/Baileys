/* AI Rich extras (htmlSection, sendHtmlApp, primitives catalog).
 * Ported from @rexxhayanasi/elaina-baileys (MessageBuilder 4.7, MIT) —
 * original credits to rexxhayanasi / Nixel are preserved. */
...
export const htmlSection = (html, { trustedSources = [], height, typename = AI_RICH_HTML_PRIMITIVE, url, sectionTypename = 'GenAIUnifiedResponseSection' } = {}) => {
    ...
    if (url !== undefined && (typeof url !== 'string' || url.trim() === '')) {
        throw new TypeError('htmlSection url must be a non-empty string (WebView base origin, e.g. https://example.com)')
    }
    return {
        // proven wire shape uses the GenAIUnifiedResponseSection wrapper;
        // pass sectionTypename: null for the bare layout form
        ...(sectionTypename ? { __typename: sectionTypename } : {}),
        ...AIRich.newLayout('Single', {
...
export const sendHtmlApp = async (sock, jid, html, { title = '', label, trustedSources, height, autoHeight: auto, scrollButtons, typename, id, url, embedded = false, screenTitle, tabHeader, bypassDownload = false, guard = 'warn', ...options } = {}) => {
    if (!sock) {
    ...
    if (url !== undefined && (typeof url !== 'string' || url.trim() === '')) {
        throw new TypeError('sendHtmlApp url must be a non-empty string (WebView base origin, e.g. https://example.com)')
    }
    ...
    if (embedded) {
        // Reproduces the proven wire shape of a hand-relayed payload:
        // body text section + the HTML primitive inside embedded_screens
        // (FOAIDNixelButtonSheets tabs). With `url` set the WebView gets a
        // real https base origin instead of an opaque/null one, so
        // WebSocket/fetch connections are allowed.
        const sheetTitle = screenTitle ?? 'Preview'

        rich.addText(label ?? sheetTitle)
        rich.addEmbeddedScreen({
            title: sheetTitle,
            content: [
                {
                    ...
                    tabs: [
                        {
                            id: 'tab_0',
                            tab_header: tabHeader ?? sheetTitle,
                            sections: [
                                htmlSection(html, { trustedSources, height, url, ...(typename ? { typename } : {}) })
                            ],
                            step_entries: []
                        }
                    ],
