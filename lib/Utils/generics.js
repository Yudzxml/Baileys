...
import * as DefaultsModule from '../Defaults/index.js';
import { DisconnectReason } from '../Types/index.js';
import { getAllBinaryNodeChildren, jidDecode } from '../WABinary/index.js';
import { sha256 } from './crypto.js';
const FALLBACK_BAILEYS_VERSION = [2, 3000, 1040735178];
const getBaileysVersion = () => {
    try {
        const v = DefaultsModule?.version;
        return Array.isArray(v) && v.length > 0 ? v : FALLBACK_BAILEYS_VERSION;
    }
    catch {
...
export const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/Yudzxml/Baileys/refs/heads/main/lib/Defaults/index.js';
    try {
        ...
        const versionMatch = text.match(/export\s+const\s+version\s*=\s*\[\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\]/);
