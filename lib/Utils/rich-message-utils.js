...
// Yudzxml@Changes 02-09-26 --- Extracted per-submessage mapper (same mapping as before, now reusable by rich-response-builder.js)
export const submessageToSection = (submessage) => {
...
export const toUnified = (submessages, uuid) => ({
    ...
    sections: submessages.map((submessage) => submessageToSection(submessage))
});
export const prepareRichResponseMessage = (content) => {
    ...
    if (Array.isArray(richResponse)) {
        ...
        if (code) {
            // Yudzxml@Fix 02-09-26 --- `language` is a const destructured field; assigning to it
            // threw "Assignment to constant variable" whenever { code } was used without a language.
