...
export async function getAudioWaveform(buffer, logger) {
	try {
		...
		if (Buffer.isBuffer(buffer)) {
			...
			throw new Error("Invalid input type for buffer");
		}
		...
		const waveform = Uint8Array.from(
			normalizedData.map(n => Math.floor(n * 100))
		);
...
/**
  referenced from @yudzxml/baileys
 */
export async function convertToOpusStream(inputStream, logger) {
    ...
    pipeline(inputStream, ff.stdin).catch(err => {
        ff.kill('SIGKILL');
        ff.stdout.destroy(err);
    });
...
export const encryptedStream = async (
    ...
    if (
        mediaType === 'audio' &&
        (isPtt === true || forceOpus === true)
    ) {
        inputStream = await convertToOpusStream(stream, logger);
        logger?.debug('converted audio stream to opus');
    }
