...
async function getAudioWaveform(buffer, logger) {
  try {
    ...
    if (Buffer.isBuffer(buffer)) {
      ...
      audioData = await toBuffer(fs.createReadStream(buffer));
    } else {
    ...
    return await new Promise((resolve, reject) => {
      ...
      ff(inputStream)
        ...
        .on('end', () => {
          ...
          const blockSize = Math.floor(amplitudes.length / bars) || 1;
          const avg = new Array(bars).fill(0);

          for (let i = 0; i < bars; i++) {
            const start = i * blockSize;
            const end = Math.min(start + blockSize, amplitudes.length);
            let sum = 0;
            for (let j = start; j < end; j++) sum += amplitudes[j];
    ...
    return new Uint8Array([
      0, 99, 0, 99, 0, 99, 0, 99, 88, 99, 0, 99, 0, 55, 0, 99,
      0, 99, 0, 99, 0, 99, 0, 99, 88, 99, 0, 99, 0, 55, 0, 99
    ]);
  }
}
