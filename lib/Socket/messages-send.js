...
import { aggregateMessageKeysNotFromMe, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, DEF_MEDIA_HOST, delay, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, generateWAMessageFromContent, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, hasValidAlbumMedia, htmlSection, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, shouldIncludeBizBinaryNode, unixTimestampSeconds, generateForwardMessageContent, downloadMediaMessage } from '../Utils/index.js';
...
export const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        // Yudzxml@Changes 02-09-26 --- Native AI Rich Response / Unified Response support
        // Accepts:
        //   sock.sendUnifiedResponse(jid, { text: "...", sections: [...] }, options?)
        //   sock.sendUnifiedResponse(jid, "text", [RichBuilder...], options?)
        // Sections are built with RichBuilder (lib/Utils/rich-response-builder.js).
        // Transport reuses the existing generateWAMessage -> relayMessage pipeline, so
        // message ID, encryption, contextInfo and event emission behave exactly like sendMessage().
        sendUnifiedResponse: async (jid, content, sections, options = {}) => {
            const meId = assertMeId(authState.creds);
            const unifiedContent = typeof content === 'string'
                ? { text: content, ...(Array.isArray(sections) ? { sections } : {}) }
                : (content || {});
            const fullMsg = await generateWAMessage(jid, { unifiedResponse: unifiedContent }, {
                logger,
                userJid: meId,
        ...
        // Yudzxml@Changes 03-09-26 --- HTML Mini App / HTML Rich Message (GenAIaeacdsnwHtmlPrimitive)
        // sock.sendHtmlApp(jid, html, options?) -> full HTML+CSS+JS travels verbatim inside the
        // GenAIaeacdsnwHtmlPrimitive payload and is rendered as a native WebView by WhatsApp Android.
        // Transport: sendUnifiedResponse pipeline (generateWAMessage -> relayMessage) followed by the
        // proven protocolMessage(type 14 MESSAGE_EDIT) edit carrying editedMessage (bypassDownload flow,
        // default ON) — mirrors the reference implementation.
        sendHtmlApp: async (jid, html, options = {}) => {
            const meId = assertMeId(authState.creds);
            const {
                bypassDownload = true,
                trustedSources,
            ...
            if (bypassDownload) {
                // bypassDownload flow (reference-verified): re-deliver the full content as
                // a MESSAGE_EDIT so the client renders the HTML Mini App natively.
                const editContent = {
                    botForwardedMessage: {
                        message: {
                            protocolMessage: {
                ...
                await relayMessage(jid, editMsg.message, {
