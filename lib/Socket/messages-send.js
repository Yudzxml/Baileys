...
import { sendHtmlApp as sendHtmlAppBuilder } from '../MessageBuilder/extras.js';
...
export const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        // UPDATED (7.6.0): now powered by the integrated Elaina-style MessageBuilder
        // (lib/MessageBuilder) instead of the legacy RichBuilder. Compat shim kept so
        // existing bot code keeps working; prefer the full API directly:
        //   const rich = new AIRich(sock).addText('hello').addSection(...); await rich.send(jid)
        sendUnifiedResponse: async (jid, content, sections, options = {}) => {
            const unifiedContent = typeof content === 'string' ? { text: content } : (content || {});
            // NOTE: the factory returns a SPREAD copy of the core sock, so the closure
            // `sock` here lacks relayMessage — always pass an enriched client object.
            const rich = new AIRich({ ...sock, relayMessage });
            if (unifiedContent.text) {
                rich.addText(unifiedContent.text);
            }
            for (const s of [...(unifiedContent.sections || []), ...(Array.isArray(sections) ? sections : [])]) {
                rich.addSection(s);
            }
            return rich.send(jid, options);
        },
        // Yudzxml@Changes 03-09-26 --- HTML Mini App / HTML Rich Message (GenAIaeacdsnwHtmlPrimitive)
        // UPDATED (7.6.0): thin shim -> MessageBuilder extras sendHtmlApp (Elaina 1.3.9 engine).
        // sock.sendHtmlApp(jid, html, options?) renders a full HTML+CSS+JS page as a native
        // WebView card in WhatsApp Android. Full option list: see examples/html-rich.js.
        sendHtmlApp: async (jid, html, options = {}) => {
            const { text, ...rest } = options;
            return sendHtmlAppBuilder({ ...sock, relayMessage }, jid, html, text !== undefined ? { label: text, ...rest } : rest);
        },
        sendMessage: async (jid, content, options = {}) => {
            ...
            // Yudzxml@Changes 13-09-26 --- group status/story via Dugong (parity with @nuiisweety/baileys):
            // supports text with backgroundColor/textColor/font plus image/video/audio/document media
            else if ('groupStatusMessage' in content && !!content.groupStatusMessage) {
                const dugong = new Dugong(waUploadToServer, relayMessage, config, sock);
                return await dugong.handleGroupStory(content, jid, options.quoted);
            }
            else {
