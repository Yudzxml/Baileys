...
// Yudzxml@Changes 13-09-26 --- ARGB color parser for group status (parity with @nuiisweety/baileys)
// Accepts "#ff6b9d" / "ff6b9d" / 8-digit hex / ARGB integer; returns unsigned int
const assertArgbColor = (color, fieldName) => {
  ...
  if (typeof color === "string") {
    ...
    if (!/^[0-9a-fA-F]{6}$|^[0-9a-fA-F]{8}$/.test(hex)) {
      throw new Boom(
        `"${fieldName}" harus berupa hex color yang valid (mis. "#ff6b9d") atau integer ARGB`,
        { statusCode: 400 }
      );
    }
  ...
  throw new Boom(
    `"${fieldName}" harus berupa string hex atau number ARGB`,
    { statusCode: 400 }
  );
};
...
export class Dugong {
  ...
  async handleGroupStory(content, jid, _quoted, options = {}) {
    ...
    // Yudzxml@Changes 13-09-26 --- full group status support (parity with @nuiisweety/baileys):
    // text with backgroundColor/textColor/font, image/video/audio/document media,
    // and audienceType -> contextInfo.statusAudienceMetadata
    ...
    if (storyData.message) {
      ...
      // text/image/video/audio/document all build through the standard content generator
    ...
    // colored text status: backgroundArgb/textArgb/font live on extendedTextMessage
    if (innerMsg.extendedTextMessage) {
      if (backgroundArgb !== undefined)
        innerMsg.extendedTextMessage.backgroundArgb = backgroundArgb;
      if (textArgb !== undefined) innerMsg.extendedTextMessage.textArgb = textArgb;
      if (font !== undefined) innerMsg.extendedTextMessage.font = font;
    }

    // Vanzxy@Fix convention: audio uses groupStatusMessage (v1) for old-client
    // compatibility (V2 causes black screen), everything else uses V2
    const finalMsg =
      msgKey === "audioMessage"
        ? {
            groupStatusMessage: {
