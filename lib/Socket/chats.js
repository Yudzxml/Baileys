...
const makeChatsSocket = (config) => {
    ...
    const appPatch = async (patchCreate) => {
        ...
        const myAppStateKeyId = authState.creds.myAppStateKeyId || Object.keys(await authState.keys.get('app-state-sync-key') || {})[0]
        
        if (!myAppStateKeyId) {
           logger.warn('Skip appPatch: appStateKey not ready')
           return
        }
	...
	const privacyTokenProp =
		props['10518'] ??
		props['privacy_token_sending_on_all_1_on_1_messages']

	if (privacyTokenProp !== undefined) {
		serverProps.privacyTokenOn1to1 =
			privacyTokenProp === 'true' ||
			privacyTokenProp === '1'
	}

	const profilePicProp =
		props['9666'] ??
		props['profile_scraping_privacy_token_in_photo_iq']
	...
	const lidIssueProp =
		props['14303'] ??
		props['lid_trusted_token_issue_to_lid']
