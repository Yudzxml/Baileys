...
There is **no verified game primitive** yet. Real game cards seen in the wild are most likely server-driven Bloks UI (`FOABloksPrimitive`) generated by Meta's own servers, which a third-party client cannot construct without the actual payload. The closest verified approximation today is markdown + table + image sections (as shown above). To find the real schema:
...
- Rendering fidelity depends on the WhatsApp client; JSON-only primitives (`image`, `divider`, `spacer`, `footerAction`) may render differently across app versions.
...
- Tests: `npm test` (covers build → serialize → deserialize → decode roundtrip, response_id/sections/primitives verification, malformed-data safety, and backward compatibility of the legacy `richResponse` path). Full socket smoke check: `node tests/smoke-socket.mjs`. Example: `examples/rich-response.js`.
